...
- [x] Создим новый кластер БД с включенной контрольной суммой страниц. Создаем таблицу **otus09**. Вставляю в таблицу **otus09** несколько значений. Выключите кластер. Измените пару байт в таблице. Включите кластер и сделайте выборку из таблицы. Что и почему произошло? как проигнорировать ошибку и продолжить работу?.
...
Создаю БД **silunsky2** и таблицу **otus09**, наполняю твблицу данными:
postgres=# CREATE DATABASE silunsky2;
CREATE DATABASE
postgres=# \c silunsky2 
psql (15.4 (Ubuntu 15.4-1.pgdg22.04+1), server 14.9 (Ubuntu 14.9-1.pgdg22.04+1))
You are now connected to database "silunsky2" as user "postgres".
silunsky2=# CREATE TABLE otus09 (i serial, amount int);
CREATE TABLE
silunsky2=# INSERT INTO otus09(amount) VALUES (100);
INSERT 0 1
silunsky2=# INSERT INTO otus09(amount) VALUES (200);
INSERT 0 1
silunsky2=# INSERT INTO otus09(amount) VALUES (400);
INSERT 0 1
silunsky2=# INSERT INTO otus09(amount) VALUES (600);
INSERT 0 1
...
----------------------
 base/16410/16412
(1 row)
postgres@postgres2:~$ ls -l /var/lib/postgresql/14/main/base/16410/16412 
-rw------- 1 postgres postgres 8192 Sep  3 07:58 /var/lib/postgresql/14/main/base/16410/16412
...
Изменим пару байт в таблице **otus09**:
